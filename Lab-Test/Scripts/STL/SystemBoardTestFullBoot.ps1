...
$psuPort = new-Object System.IO.Ports.SerialPort COM6,115200,None,8,1
...
$testCycles=1 # Number of test blocks to run
$testIterations=500 # Number of tests in each block to run
...
for($t=0;$t -lt $testCycles;$t++){
    ...
    for($r=0;$r -lt $testIterations;$r++){
        ...
        if($fullboot -and $bootdetect){
            ...
            write-host Ping test has $dataLoss dataloss
        ...
        $result | Add-member -MemberType NoteProperty -Name booted -Value $bootdetect
        $result | Add-member -MemberType NoteProperty -Name dataloss -Value $dataLoss
